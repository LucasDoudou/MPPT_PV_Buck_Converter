 ...
 */
 ...
 // Measure var. MPPT_power, MPPT_duty, flag_sweep
...
float MPP_Duty = 0.55;
float MPP_Vin; // Maximum power point duty cycle and input voltage
...
volatile int flag_sweep = 0; // Flag to sweep duty
...
int main(void)
{
    ...
    while (1) // main circuit setup
    {
        ...
        // adc_vout = ((float)adc_raw_vout / 4095.0) * 3.0; // float from 12-bit value to 3V
        // // adc_vin = 0.0007 * adc_raw + 0.002;

        // // TODO: sense iout + calculate pout
        // adc_raw_iout = AdccResultRegs.ADCRESULT0;
        // adc_iout = ((float)adc_raw_iout / 4095.0) * 3.0;
        ...
        DELAY_US(10000); // Insert a delay longer than the switching period to ensure that the duty ratio
